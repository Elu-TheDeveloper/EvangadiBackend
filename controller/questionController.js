...
return res.status(StatusCodes.BAD_REQUEST).json({ msg:"All fields are required"})
    ...
    try {
        ...
        "INSERT INTO questions(questionid, userid, title, description, tag )VALUES(?, ?, ?, ?, ?)",[questionid, userid,  title, description, tag]
        );
        res.status(StatusCodes.CREATED).json({msg: "Question posted succesfully"})
    } catch (error) {
        console.log(error.message);
        res.status(StatusCodes.BAD_REQUEST).json({msg:"Error posting Question"})
